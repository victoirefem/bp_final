...
AML_DIR = EXAMPLES_DIR / 'aml'
ML_DIR = PROJECT_ROOT / 'ml_tests'
...
def run_mpspdz_circuit(mpspdz_circuit_path: Path, num_parties: int) -> dict[str, int]:
    ...
    try:
        ...
        print("MP-SPDZ Error Output:\n")
        print(e.stderr)
        print("MP-SPDZ Standard Output:\n")
        print(e.stdout)
        raise
...
def workflow(circuit_name: str, dir_name: str, mpc_settings_change: bool = False, is_ml: bool = False, public_inputs: dict = None):
    ...
    if is_ml:
        circuit_dir = ML_DIR / dir_name
    else:
        circuit_dir = EXAMPLES_DIR / dir_name 


    circom_path = circuit_dir / f"{circuit_name}.circom"

    args_str = ""
    if public_inputs and circuit_name == "aml":
        bw = public_inputs.get("0.bw", 0)
        pml = public_inputs.get("0.pml", 0)
        args_str = f"--bw {bw} --pml {pml}"

    mpc_settings_name = ''
    if mpc_settings_change:
        mpc_settings_name = f"mpc_settings_{circuit_name}.json"
    else:
        mpc_settings_name = "mpc_settings.json"

    py_name = ''
    if mpc_settings_change:
        py_name = f"{circuit_name}.py"
    else:
        py_name = f"{dir_name}.py"

    # Step 1b: run circuit script
    # python {circuit}.py
    code = os.system(f"cd {circuit_dir} && python3 {py_name} {args_str}")
    if code != 0:
        raise ValueError(f"Failed to run cd {circuit_dir} && python3 {circuit_name}.py. Error code: {code}")

    mpc_settings_path = circuit_dir / f"{mpc_settings_name}"
    with open(mpc_settings_path, 'r') as f:
        mpc_settings = json.load(f)
    ...
    with open(output_dir / "benchmark.json", 'w') as fp:
